...
int main(int argc, char const *argv[])
{
    ...
    //connected
    printf("<Hospital B> is now connected to the admission office\n");
    ...
    for (int i = 0; i < hospitalInfo.size(); i++)
    {
        ...
        printf("<Hospital B> has sent <%c%c> to the admission office\n", hospitalInfo[i][0], hospitalInfo[i][1]);
    }
    send(sock, x.c_str(), strlen(x.c_str()), 0);

    //connected end
    printf("Updating the admission office is done for <Hospital B>\n");
