...
std::vector<std::string> readInfo()
{
    ...
    std::ifstream file("input/student4.txt");
    if (file.is_open())
    {
        std::string line;
...
int main(int argc, char const *argv[])
{
    ...
    std::cout << "<Student 4> has TCP port " << ntohs(current.sin_port) << " and IP address " << inet_ntoa(current.sin_addr) << std::endl;

    //read information and send it
    std::vector<std::string> hospitalInfo = readInfo();
    std::string x;
    x.append("Student4:" + hospitalInfo[0]);
    ...
    //connected end
    printf("Completed sending application for <Student 4>\n");
    ...
    if (buffer[0] == '1')
    {
        printf("<Student 4> has received the reply from the health center\n");
        close(sock);
        ...
        std::cout << "<Student 4> has UDP port " << htons((servaddr.sin_port)) << " and IP address " << inet_ntoa(servaddr.sin_addr) << std::endl;
        ...
        if (buffer[0] == 'N')
        {
            printf("<Student 4> has received the application result, the result is: rejected\n");
        }
        else
            printf("<Student 4> has received the application result, the result is: %s\n", buffer);
    }
    return 0;
}
